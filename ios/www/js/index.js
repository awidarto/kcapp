        ...
        if (!hash || hash === '#home') {
            ...
            if (match) {
                this.store.findArticleBySection('whatson', function(articles){
                    ...
                    if(this.whatPage){
                        self.slidePage(this.whatPage);
                    }else{
                        this.whatPage = new FeedView(viewObj).render();
                        self.slidePage(this.whatPage);
                    }
                    self.setactive('home');
                });
            ...
            if (match) {
                ...
                this.store.findArticleBySection('promo', function(articles){
                    ...
                    viewObj.feeds = articles;
                    if(this.promoPage){
                        self.slidePage(this.promoPage);
                    }else{
                        this.promoPage = new FeedView(viewObj).render();
                        self.slidePage(this.promoPage);
                    }
                    self.setactive('home');
                });
            ...
            if (match) {
                if(this.dirPage){
                    self.slidePage(this.dirPage);
                }else{
                    this.dirPage = new DirectoryView(this.store).render();
                    self.slidePage(this.dirPage);
                }
            ...
            if (match) {
                self.slidePage(new GamesView("main").render());
                self.setactive('games');
            }            

            match = hash.match(app.gamesChoiceURL);
            if (match) {
                self.slidePage(new GamesView("choice").render());
                self.setactive('games');
            }            

            match = hash.match(app.gamesTsearchURL);
            if (match) {
                self.slidePage(new GamesView("tsearch").render());
                self.setactive('games');
            }            

            match = hash.match(app.gamesLogoURL);
            if (match) {
                self.slidePage(new GamesView("logo").render());
                self.setactive('games');
            }            
